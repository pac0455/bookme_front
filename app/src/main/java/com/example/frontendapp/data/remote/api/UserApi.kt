...
interface UserApi {
    ...
    @GET("$controller/sendMail/{id}")
    suspend fun sendAuthenticationCode(@Path("id") userId: String): Response<SingleMessageResponse>
    @POST("$controller/confirm")
    suspend fun confirmEmail(@Body model: ConfirmMailDTO): Response<SingleMessageResponse>
}